...
use App\Models\{CartItem, Order, Product, RewardClaim, User, UserAddress};
...
class CartController extends Controller
{
    ...
    private function availableVoucherClaims(User $user)
    {
        if (! Schema::hasColumn('reward_claims', 'voucher_code')) {
            return collect();
        }
        ...
        return RewardClaim::query()
            ...
            ->when(Schema::hasColumn('reward_claims', 'status'), fn($q) => $q->where('status', 'available'))
            ->when(Schema::hasColumn('reward_claims', 'used_at'), fn($q) => $q->whereNull('used_at'))
            ->when(Schema::hasColumn('reward_claims', 'expires_at'), fn($q) => $q->where(function($qq){
    ...
    private function voucherLabel(RewardClaim $claim): string
    {
        $type = (string) ($claim->discount_type ?? 'fixed');
        $value = (int) ($claim->discount_value ?? 0);
        $max = (int) ($claim->max_discount ?? 0);

        return match ($type) {
            'percent' => $value . '% off' . ($max > 0 ? ' maks. Rp ' . number_format($max, 0, ',', '.') : ''),
            'free_shipping' => 'Gratis ongkir' . ($max > 0 ? ' maks. Rp ' . number_format($max, 0, ',', '.') : ''),
            default => 'Potongan Rp ' . number_format($value, 0, ',', '.'),
        };
    }

    private function calculateVoucherDiscount(?RewardClaim $claim, int $subtotal, int $shippingCost): int
    {
        if (! $claim) return 0;

        $minOrder = (int) ($claim->min_order ?? 0);
        if ($minOrder > 0 && $subtotal < $minOrder) return 0;

        $type = (string) ($claim->discount_type ?? 'fixed');
        $value = max(0, (int) ($claim->discount_value ?? 0));
        $max = (int) ($claim->max_discount ?? 0);

        $discount = match ($type) {
            'percent' => (int) floor($subtotal * min(100, $value) / 100),
        ...
        return max(0, min($discount, $subtotal + $shippingCost));
    }
    ...
    public function checkout(RajaOngkirService $rajaOngkir)
    {
        ...
        $voucherClaims = $this->availableVoucherClaims($user);

        return view('pages.checkout', compact('user', 'cartItems', 'subtotal', 'shippingCost', 'total', 'address', 'shippingOptions', 'defaultWeight', 'rajaOngkirConfigured', 'voucherClaims'));
    }
    ...
    public function pay(Request $request, MidtransSnapService $midtrans, RajaOngkirService $rajaOngkir)
    {
        ...
        try {
            ...
            $order = DB::transaction(function () use ($user, $cartItems, $data, $shippingOption, $weight, $midtrans) {
                ...
                if ($voucherCode !== '') {
                    if (! Schema::hasColumn('reward_claims', 'voucher_code')) {
                        throw new \RuntimeException('Database voucher belum diperbarui. Jalankan migration/SQL patch voucher dulu.');
                    }

                    $voucherClaim = RewardClaim::where('user_id', $user->id)
                        ->where('voucher_code', $voucherCode)
                        ->lockForUpdate()
                        ->first();

                    if (! $voucherClaim) {
                        throw new \RuntimeException('Kode voucher tidak ditemukan di akun kamu.');
                    }

                    if (Schema::hasColumn('reward_claims', 'status') && $voucherClaim->status !== 'available') {
                        throw new \RuntimeException('Voucher ini sudah dipakai atau tidak aktif.');
                    }

                    if (Schema::hasColumn('reward_claims', 'used_at') && $voucherClaim->used_at) {
                        throw new \RuntimeException('Voucher ini sudah dipakai.');
                    }

                    if (Schema::hasColumn('reward_claims', 'expires_at') && $voucherClaim->expires_at && now()->gt($voucherClaim->expires_at)) {
                        throw new \RuntimeException('Voucher ini sudah kadaluarsa.');
                    }

                    if ((int) ($voucherClaim->min_order ?? 0) > 0 && $subtotal < (int) $voucherClaim->min_order) {
                        throw new \RuntimeException('Minimal belanja untuk voucher ini adalah Rp '.number_format((int) $voucherClaim->min_order, 0, ',', '.').'.');
                    }

                    $voucherDiscount = $this->calculateVoucherDiscount($voucherClaim, $subtotal, $shippingCost);
                    if ($voucherDiscount <= 0) {
                        throw new \RuntimeException('Voucher belum bisa dipakai untuk checkout ini.');
                    }
                }
                ...
                if (Schema::hasColumn('orders', 'voucher_claim_id')) $orderPayload['voucher_claim_id'] = $voucherClaim?->id;
                if (Schema::hasColumn('orders', 'voucher_code')) $orderPayload['voucher_code'] = $voucherClaim?->voucher_code;
                if (Schema::hasColumn('orders', 'voucher_discount')) $orderPayload['voucher_discount'] = $voucherDiscount;

                $order = Order::create($orderPayload);

                if ($voucherClaim) {
                    $voucherUpdate = [];
                    if (Schema::hasColumn('reward_claims', 'status')) $voucherUpdate['status'] = 'used';
                    if (Schema::hasColumn('reward_claims', 'used_at')) $voucherUpdate['used_at'] = now();
                    if (Schema::hasColumn('reward_claims', 'order_id')) $voucherUpdate['order_id'] = $order->id;
                    if ($voucherUpdate) $voucherClaim->update($voucherUpdate);
                }
