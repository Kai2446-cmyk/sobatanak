...
/* PATCH: transisi slide login/register/forgot + sembunyikan notifikasi info biru untuk user */
.auth-alert-soft{display:none!important}
.auth-split-card.auth-slide-leave-right{animation:authSlideLeaveRight .30s cubic-bezier(.2,.8,.2,1) forwards!important}
.auth-split-card.auth-slide-leave-left{animation:authSlideLeaveLeft .30s cubic-bezier(.2,.8,.2,1) forwards!important}
.auth-split-card.auth-slide-enter-right{animation:authSlideEnterRight .34s cubic-bezier(.2,.8,.2,1) both!important}
.auth-split-card.auth-slide-enter-left{animation:authSlideEnterLeft .34s cubic-bezier(.2,.8,.2,1) both!important}
@keyframes authSlideLeaveRight{to{opacity:0;transform:translateX(44px) scale(.985)}}
@keyframes authSlideLeaveLeft{to{opacity:0;transform:translateX(-44px) scale(.985)}}
@keyframes authSlideEnterRight{from{opacity:0;transform:translateX(44px) scale(.985)}to{opacity:1;transform:translateX(0) scale(1)}}
@keyframes authSlideEnterLeft{from{opacity:0;transform:translateX(-44px) scale(.985)}to{opacity:1;transform:translateX(0) scale(1)}}
.auth-split-section.auth-is-changing{pointer-events:none}
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if (!card) return;
    const entry = sessionStorage.getItem('sobatanak_auth_slide_entry');
    if (entry) {
        card.classList.remove('auth-animate-in');
        card.classList.add(entry === 'left' ? 'auth-slide-enter-left' : 'auth-slide-enter-right');
        sessionStorage.removeItem('sobatanak_auth_slide_entry');
    }
    document.querySelectorAll('a.auth-slide-nav').forEach(function(link){
        link.addEventListener('click', function(e){
            const href = link.getAttribute('href');
            if (!href || href === '#' || link.target === '_blank' || e.ctrlKey || e.metaKey || e.shiftKey || e.altKey) return;
            e.preventDefault();
            const dir = link.dataset.authSlide === 'left' ? 'left' : 'right';
            document.querySelector('.auth-split-section')?.classList.add('auth-is-changing');
            card.classList.remove('auth-animate-in','auth-slide-enter-left','auth-slide-enter-right');
            card.classList.add(dir === 'left' ? 'auth-slide-leave-left' : 'auth-slide-leave-right');
            sessionStorage.setItem('sobatanak_auth_slide_entry', dir === 'left' ? 'right' : 'left');
