...
export const GameplayScreen: React.FC<GameplayScreenProps> = ({
  ...
  const cleanPercentageRef = useRef<number>(0);
  ...
  useEffect(() => {
    ...
    cleanPercentageRef.current = 0;
    gameEndedRef.current = false;
  }, []);

  // Simpan nilai kebersihan terbaru tanpa me-reset interval timer saat user menyikat.
  useEffect(() => {
    cleanPercentageRef.current = cleanPercentage;
  }, [cleanPercentage]);

  // Hitung mundur stabil. Interval hanya dibuat ulang saat status kemenangan berubah,
  // bukan setiap kali noda/persentase berubah akibat gerakan mouse atau sentuhan.
  useEffect(() => {
    if (isVictoryCelebration || gameEndedRef.current) return;
    ...
    const timer = window.setInterval(() => {
      setTimeLeft((prev) => {
        ...
        if (nextTime === 0 && !gameEndedRef.current) {
          gameEndedRef.current = true;
          window.setTimeout(() => onGameEnd(cleanPercentageRef.current), 0);
        }
  ...
  }, [isVictoryCelebration, onGameEnd, setTimeLeftParent]);
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center select-none">
      ...
      <div
        ...
        className="relative w-full h-[410px] overflow-hidden p-4 flex flex-col items-center justify-between rounded-2xl bg-[#E0F7FA] cursor-none"
      >
        ...
        {activeBrushPos && (
          <div
            ...
            style={{
              ...
              transform: `translate(-50%, -68%) ${isBrushing ? 'rotate(-4deg) scale(1.04)' : 'rotate(0deg)'}`,
            }}
          >
            ...
            <div className="relative">
              ...
              {/* Selena sebagai kursor sikat gigi */}
              <img
                src="/games/sikat-gigi/dist/selena-sikat.png"
                alt="Selena sedang menyikat gigi"
                draggable={false}
                style={{ width: "82px", maxWidth: "18vw" }}
                className="h-auto select-none drop-shadow-[0_8px_12px_rgba(30,41,57,0.28)]"
              />
            </div>
          </div>
        )}
