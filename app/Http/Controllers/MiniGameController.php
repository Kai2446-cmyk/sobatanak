...
class MiniGameController extends Controller
{
    ...
    private function requireLoginBeforePlay()
    {
        if (session('user_id')) {
            return null;
        }
        ...
        return redirect()
            ->route('login', ['redirect_to' => request()->getRequestUri()])
            ->withErrors(['login' => 'Silakan login dulu untuk bermain game dan menyimpan poin SobatAnak.']);
    }

    public function tapTapKuman()
    {
        if ($redirect = $this->requireLoginBeforePlay()) return $redirect;
