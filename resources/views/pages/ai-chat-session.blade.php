    ...
    <aside class="gemini-sidebar">
        ...
        <div class="gemini-sidebar-user">
            ...
            <div class="gemini-sidebar-avatar">
                ...
                @else
                    <span aria-label="Belum ada foto profil"></span>
                @endif
            </div>
        ...
        <div class="gemini-chat-area" id="geminiChatArea">
            ...
            @else
                @foreach($messages as $m)
                    ...
                    @endphp
                    @php
                        $chatUserAvatarUrl = $authUser?->avatar_url;
                    @endphp
                    <div class="gemini-message {{ $m->role === 'user' ? 'user' : 'assistant' }}">
                        <div class="gemini-avatar {{ $m->role === 'assistant' ? 'ai-logo' : '' }} {{ $m->role === 'user' && $chatUserAvatarUrl ? 'has-photo' : '' }}">
                            @if($m->role === 'user')
                                @if($chatUserAvatarUrl)
                                    <img src="{{ $chatUserAvatarUrl }}" alt="{{ $authUser?->name ?? 'User' }}" onerror="this.style.display='none';this.parentElement.textContent='{{ strtoupper(substr($authUser?->name ?? 'U', 0, 1)) }}';">
                                @else
...
.gemini-avatar.ai-logo{background:#fff!important;border:1px solid var(--sa-line);padding:.38rem;overflow:hidden}.gemini-avatar.ai-logo img{width:100%;height:100%;object-fit:contain}.gemini-avatar.has-photo{padding:0!important;overflow:hidden;background:#fff!important;border:2px solid #D4EEEC}.gemini-avatar.has-photo img{width:100%;height:100%;object-fit:cover;border-radius:inherit;display:block}
...
/* PATCH: rapikan history chat AI agar compact seperti ChatGPT/Gemini */
.gemini-recent-list{
    ...
    overflow-y:auto!important;
    overflow-x:hidden!important;
    padding:.05rem .1rem .75rem!important;
...
.gemini-session-link{
    ...
    min-width:0!important;
    display:block!important;
    padding:.72rem .45rem .72rem .9rem!important;
    white-space:nowrap!important;
    overflow:hidden!important;
    text-overflow:ellipsis!important;
    line-height:1.15!important;
}
.gemini-delete-session-form{
    flex:0 0 auto!important;
    margin:0!important;
    display:flex!important;
    align-items:center!important;
...
/* PATCH FINAL: posisi sidebar dikembalikan ke atas, hanya history dibuat compact */
.gemini-side-brand{order:1!important;}
.gemini-sidebar-user{
    order:2!important;
    ...
    background:linear-gradient(135deg,rgba(255,255,255,.72),rgba(234,253,250,.76))!important;
}
.gemini-back-home{order:3!important;}
.gemini-new-chat{order:4!important;}
.gemini-recent-title{order:5!important;margin-top:.65rem!important;}
.gemini-recent-list{
    order:6!important;
    flex:1 1 auto!important;
...
.gemini-session-link{
    padding:.64rem .45rem .64rem .82rem!important;
...
(() => {
    ...
    @php($userAvatarUrl = $authUser?->avatar_url)
