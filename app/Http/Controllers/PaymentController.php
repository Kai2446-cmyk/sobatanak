...
use App\Models\{CartItem, Order, Product, RewardClaim, User};
...
class PaymentController extends Controller
{
    ...
    private function restoreVoucherIfOrderNotPaid(Order $order): void
    {
        if (! Schema::hasColumn('orders', 'voucher_claim_id') || empty($order->voucher_claim_id)) {
            return;
        }

        $claim = RewardClaim::whereKey($order->voucher_claim_id)->lockForUpdate()->first();
        if (! $claim) return;

        $payload = [];
        if (Schema::hasColumn('reward_claims', 'status')) $payload['status'] = 'available';
        if (Schema::hasColumn('reward_claims', 'used_at')) $payload['used_at'] = null;
        if (Schema::hasColumn('reward_claims', 'order_id')) $payload['order_id'] = null;
        if ($payload) $claim->update($payload);
    }
    ...
    private function markExpiredIfNeeded(Order $order): bool
    {
        ...
        DB::transaction(function () use ($order) {
            ...
            $this->restoreVoucherIfOrderNotPaid($locked);
        });
