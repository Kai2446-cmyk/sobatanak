...
export const EndScreen: React.FC<EndScreenProps> = ({ score, onRestart }) => {
  ...
  if (score === 0) {
    ...
    feedbackMessage = 'Yuk, ambil sikat gigimu dan kita coba usir kumannya bersama-sama! 🪥✨';
  } else if (score >= 1 && score <= 49) {
    crownLabel = 'USAHA BAGUS! 👍';
    headlineText = 'Ayo Gosok Lebih Bersih!';
    feedbackMessage = `Yuk, main lagi dan sikat lebih bersih sampai kumannya hilang semua! 🦷`;
  } else if (score >= 50 && score <= 99) {
    crownLabel = 'HEBAT SEKALI! 🎉';
    headlineText = 'Gigi Mumu Jauh Lebih Bersih!';
    feedbackMessage = `Ingat ya, sisa kuman yang sedikit lagi itu harus terus disikat agar gigimu tetap kuat! 🪥💎`;
  } else {
