...
const GAME_ASSET_BASE = "/games/keranjang-sehat/dist";

// Data template item beserta custom SVG fallback yang ceria & ramah anak
const ITEM_TEMPLATES = [
  {
    name: "Apel Segar",
    img: `${GAME_ASSET_BASE}/apel.png`,
  ...
  {
    name: "Wortel Manis",
    img: `${GAME_ASSET_BASE}/wortel.png`,
    type: "healthy" as const,
  ...
  {
    name: "Permen Manis",
    img: `${GAME_ASSET_BASE}/permen.png`,
...
export default function App() {
  ...
  const [basketMood, setBasketMood] = useState<"neutral" | "catch" | "miss">("neutral");
  const [isStartCharacterActive, setIsStartCharacterActive] = useState(false);
  ...
  const [endMascot, setEndMascot] = useState<"solis" | "selena" | null>(null);
  const [showEndStamp, setShowEndStamp] = useState(false);
  ...
  const basketMoodTimerRef = useRef<number | null>(null);
  ...
  const showBasketMood = (mood: "neutral" | "catch" | "miss", duration = 750) => {
    setBasketMood(mood);
    if (basketMoodTimerRef.current) window.clearTimeout(basketMoodTimerRef.current);
    if (mood !== "neutral") {
      basketMoodTimerRef.current = window.setTimeout(() => setBasketMood("neutral"), duration);
    }
  };

  const currentBasketImage =
    basketMood === "catch"
      ? `${GAME_ASSET_BASE}/selena-catch.png`
      : basketMood === "miss"
      ? `${GAME_ASSET_BASE}/selena-miss.png`
      : `${GAME_ASSET_BASE}/selena-keranjang.png`;
  ...
  useEffect(() => {
  const bgm = new Audio(`${GAME_ASSET_BASE}/audio/backsound-keranjang-sehat.mp3`);
...
const handleStartGame = () => {
  setIsStartCharacterActive(true);
  setEndMascot(null);
  setShowEndStamp(false);

  window.setTimeout(() => {
    setScore(0);
    setTimeLeft(30);
    setBasketX(35);
    setBasketMood("neutral");
    itemsRef.current = [];
    setItems([]);

    setGameState("playing");
    setIsStartCharacterActive(false);
    playBacksound();
  }, 180);
};
...
const handleRestart = () => {
  ...
  useEffect(() => {
    ...
    const chosen = Math.random() < 0.5 ? "solis" : "selena";
    setEndMascot(chosen);
    setShowEndStamp(false);

    const stampTimer = window.setTimeout(() => {
      setShowEndStamp(true);
    }, 420);
      ...
      <main className="grow w-full max-w-4xl mx-auto px-4 py-6 md:py-8 flex flex-col">
            ...
            >
              ...
              <div className="w-full max-w-4xl flex flex-col gap-4 mb-4">
                ...
                <div className="w-full max-w-4xl flex items-center justify-between mb-4 mt-2">
                  ...
                  <div className="flex items-center gap-4">
                    <img
                      src={`${GAME_ASSET_BASE}/logo sobat anak.png`}
                      alt="Sobat Anak"
                  ...
                  <div className="bg-white border-4 border-orange-500 rounded-3xl p-3 md:p-4 flex items-center justify-center md:justify-start shadow-lg relative h-16 md:h-28 select-none transition-all duration-200 hover:scale-[1.02]">
                    <div className="flex items-center space-x-1.5 md:space-x-3.5">
                      <div className="w-8 h-8 md:w-14 md:h-14 rounded-2xl flex items-center justify-center border-2 bg-[#FFF0E6] border-[#FDBA74] shrink-0">
                        <Timer className="w-4 h-4 md:w-7 md:h-7 text-[#F97316]" strokeWidth={2.5} />
                      </div>

                      <div>
                        <p className="text-[10px] md:text-[11px] font-extrabold uppercase tracking-wider md:tracking-widest leading-none text-[#F97316]">
                          Waktu Tersisa
                        </p>

                        <div className="flex items-baseline mt-0.5 md:mt-1">
                          <span className="text-2xl md:text-5xl font-black leading-none text-[#F97316] font-display">
                            {timeLeft}
                          </span>
                          <span className="text-sm md:text-xl font-black ml-1 md:ml-2 leading-none text-[#F97316] font-display">
                            detik
                          </span>
                        </div>
                      </div>
              ...
              <div
                ...
                style={{ touchAction: "none", height: "450px", minHeight: "450px" }}
              >
                ...
                <div className="absolute inset-0 bg-[#E0F2FE]/40 pointer-events-none">
                  ...
                  <div className="absolute bottom-0 inset-x-0 h-[22px] bg-[#A7DDB9] border-t-2 border-[#8CCFA4] shadow-[0_-3px_8px_rgba(34,197,94,0.08)] z-[2]"></div>
                </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {gameState === "start" && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, scale: 1 }}
                      exit={{ opacity: 0 }}
                      className="absolute inset-0 z-10 flex items-center justify-center p-4 md:p-8 bg-white/80 backdrop-blur-[2px]"
                    >
                      <div className="start-keranjang-layout w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 md:gap-10 px-2 md:px-6">
                        <div className="start-keranjang-copy order-2 md:order-1 flex-1 max-w-xl text-center md:text-left">
                          <h1 className="text-3xl md:text-5xl font-black font-display text-slate-800 tracking-tight leading-tight mb-4 uppercase">
                            Keranjang Sehat
                          </h1>

                          <p className="text-slate-600 font-bold text-sm md:text-base mb-6 md:mb-8 max-w-md leading-relaxed mx-auto md:mx-0">
                            Tangkap makanan sehat seperti apel, wortel, dan susu. Hindari permen, donat, dan soda agar skormu tetap tinggi!
                          </p>

                          <button
                            onClick={handleStartGame}
                            onMouseEnter={() => setIsStartCharacterActive(true)}
                            onMouseLeave={() => setIsStartCharacterActive(false)}
                            onPointerDown={() => setIsStartCharacterActive(true)}
                            onPointerUp={() => setIsStartCharacterActive(false)}
                            onFocus={() => setIsStartCharacterActive(true)}
                            onBlur={() => setIsStartCharacterActive(false)}
                            className="bg-green-500 hover:bg-green-600 text-white text-lg md:text-2xl font-black font-display px-8 md:px-12 py-3 md:py-4 rounded-full shadow-[0_6px_0_0_#166534] active:shadow-none active:translate-y-1 transition-all uppercase tracking-widest cursor-pointer inline-flex items-center gap-2 md:gap-3 hover:scale-105 active:scale-95 duration-150"
                          >
                        ...
                        <div className="start-keranjang-visual order-1 md:order-2 flex-1">
                          <img src={`${GAME_ASSET_BASE}/soda.png`} alt="Soda" className="start-float-item item-soda" />
                          <img src={`${GAME_ASSET_BASE}/susu.png`} alt="Susu" className="start-float-item item-susu" />
                          <img src={`${GAME_ASSET_BASE}/permen.png`} alt="Permen" className="start-float-item item-permen" />
                          <img src={`${GAME_ASSET_BASE}/donat.png`} alt="Donat" className="start-float-item item-donat" />
                          <img src={`${GAME_ASSET_BASE}/wortel.png`} alt="Wortel" className="start-float-item item-wortel" />
                          <img src={`${GAME_ASSET_BASE}/apel.png`} alt="Apel" className="start-float-item item-apel" />

                          <div className="start-selena-wrap">
                            <img
                              src={
                                isStartCharacterActive
                                  ? `${GAME_ASSET_BASE}/selena-keranjangsehat2.png`
                                  : `${GAME_ASSET_BASE}/selena-keranjangsehat1.png`
                              }
                              alt="Selena Keranjang Sehat"
                              className={`start-selena-image h-auto object-contain ${
                                isStartCharacterActive ? "start-selena-image-active" : ""
                              }`}
                              draggable={false}
                            />
                          </div>
                  ...
                  {gameState === "playing" && (
                    <div key="screen-play" className="absolute inset-0">
                      ...
                      {lastCrash && (
                        <div
                          className="absolute pointer-events-none z-30 text-center font-display text-lg"
                          style={{
                            left: `calc(${basketX}% + 18%)`,
                            bottom: "125px",
                            transform: "translateX(-50%)",
                            animation: "scorePopupUp 0.6s ease-out forwards"
                          }}
                        >
                          <span className={`inline-flex items-center justify-center min-w-14 bg-white border-2 border-slate-100 rounded-full px-3 py-1.5 shadow-lg ${lastCrash.color}`}>
                            {lastCrash.text}
                          </span>
                        </div>
                      )}
                      ...
                      {/* CHARACTER: SELENA SEBAGAI KERANJANG */}
                      <div
                        className="absolute cursor-grab active:cursor-grabbing z-10 select-none transition-transform duration-150"
                        style={{
                          left: `${basketX}%`,
                          bottom: "0px",
                          width: "18%",
                      ...
                      >
                        <img
                          src={currentBasketImage}
                          alt="Selena Keranjang"
                          className={`w-full h-auto object-contain select-none pointer-events-none filter drop-shadow-[0_10px_16px_rgba(93,61,136,0.22)] ${
                            basketMood === "catch" ? "scale-105" : basketMood === "miss" ? "scale-[0.98]" : ""
                          }`}
                          draggable={false}
                        />
                      </div>
                  ...
                  {gameState === "end" && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, scale: 1 }}
                      exit={{ opacity: 0 }}
                      className="absolute inset-0 z-20 flex flex-col items-center justify-center px-4 py-5 md:px-6 md:py-6 bg-white text-center overflow-hidden"
                    >
                      <div className="tap-end-hero tap-end-hero-reset">
                        {endMascot && (
                          <img
                            src={endMascot === 'solis' ? '/games/keranjang-sehat/end-assets/solis-jempol.png' : '/games/keranjang-sehat/end-assets/selena-jempol.png'}
                            alt={endMascot === 'solis' ? 'Solis jempol' : 'Selena jempol'}
                            className="tap-end-mascot tap-end-mascot-reset"
                          />
                        )}

                        {showEndStamp && (
                          <div className="tap-end-stamp tap-end-stamp-reset">
                            GAME SELESAI!
                          </div>
                        )}
                      </div>

                      <h2 className="text-2xl md:text-4xl font-black text-slate-800 mb-2 select-none">
                        Skor Akhir: <span className="text-blue-600 underline decoration-blue-300 decoration-wavy tabular-nums">{score}</span>
                      </h2>

                      <p className="text-base md:text-xl font-black italic text-slate-600 mb-3 md:mb-4 text-center select-none max-w-4xl">
                        "{evalInfo.desc}"
                      </p>

                      <button 
                        onClick={handleRestart}
                        className="bg-green-500 hover:bg-green-600 text-white text-lg md:text-2xl font-black px-8 md:px-12 py-3 md:py-4 rounded-full shadow-[0_6px_0_0_#166534] active:shadow-none active:translate-y-1 transition-all uppercase tracking-widest cursor-pointer flex items-center gap-2 md:gap-3 hover:scale-105 active:scale-95 duration-150 mt-0 md:mt-1"
                      >
