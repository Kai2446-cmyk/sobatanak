...
<section class="max-w-7xl mx-auto px-6 md:px-12 py-12 checkout-official-page">
  ...
  @php
    ...
    $initialVoucherCode = old('voucher_code', '');
    $initialTotal = (int) $subtotal + $initialShippingCost;
    $voucherOptions = collect($voucherClaims ?? [])->map(function($claim){
        $type = $claim->discount_type ?? 'fixed';
        $value = (int) ($claim->discount_value ?? 0);
        $max = (int) ($claim->max_discount ?? 0);
        $min = (int) ($claim->min_order ?? 0);
        $text = $type === 'percent'
        ...
        return [
            ...
            'max' => $max,
            'min' => $min,
            'expires_at' => $claim->expires_at ? $claim->expires_at->format('d M Y') : null,
            'label' => $claim->reward_name.' • '.$text.' • Kode: '.$claim->voucher_code.($claim->expires_at ? ' • Exp: '.$claim->expires_at->format('d M Y') : ''),
            'min_label' => $min > 0 ? 'Min. belanja Rp '.number_format($min,0,',','.') : 'Tanpa minimal belanja',
        ];
    })->values();
  @endphp
  ...
  <form method="POST" action="{{ route('checkout.pay') }}" class="checkout-grid" id="checkoutOfficialForm">
    ...
    <div class="checkout-left">
      ...
      <div class="checkout-card">
        <div class="checkout-card-head">
          ...
          <div>
            <p class="checkout-kicker">Voucher</p>
            <h2>Pakai voucher kamu</h2>
          </div>
        </div>
        ...
        <div class="checkout-voucher-box">
          <label>
            ...
            <select name="voucher_code" id="voucherCodeSelect">
              <option value="">Tidak pakai voucher</option>
              @foreach($voucherOptions as $voucher)
                <option value="{{ $voucher['code'] }}" @selected($initialVoucherCode === $voucher['code'])>
                  {{ $voucher['label'] }}
                </option>
              @endforeach
            </select>
          </label>
          <p id="voucherHelp">{{ $voucherOptions->count() ? 'Pilih voucher yang sudah kamu klaim dari menu Reward.' : 'Belum ada voucher aktif. Tukar poin di menu Reward dulu.' }}</p>
        </div>
      </div>
    ...
    <aside class="checkout-summary-card">
      ...
      <div class="summary-line voucher-summary-line" id="voucherSummaryLine" hidden>
        <span>Voucher</span>
        <b id="summaryVoucherDiscount">- Rp 0</b>
      </div>
...
.checkout-voucher-box{border:1.5px solid var(--border);border-radius:1.15rem;background:#F7FBFA;padding:1rem}.checkout-voucher-box label{display:flex;flex-direction:column;gap:.45rem}.checkout-voucher-box span{font-weight:1000;color:var(--fg)}.checkout-voucher-box select{width:100%;border:1.5px solid var(--border);border-radius:1rem;background:#fff;padding:.9rem 1rem;font-weight:950;color:var(--fg);outline:0}.checkout-voucher-box select:focus{border-color:#4BBFB0;box-shadow:0 0 0 4px rgba(75,191,176,.13)}.checkout-voucher-box p{margin:.65rem 0 0;color:var(--muted);font-size:.83rem;font-weight:850;line-height:1.45}.voucher-summary-line b{color:#2f9e92!important}
</style>
<script>
document.addEventListener('DOMContentLoaded', function(){
  ...
  const vouchers = @json($voucherOptions);
  const selectedVoucherOld = @json($initialVoucherCode);
  ...
  const voucherCodeSelect = document.getElementById('voucherCodeSelect');
  const voucherHelp = document.getElementById('voucherHelp');
  ...
  function getSelectedVoucher(){
    const code = voucherCodeSelect ? voucherCodeSelect.value : '';
    return vouchers.find(v => v.code === code) || null;
  }

  function calculateVoucherDiscount(voucher, shippingCost){
    if(!voucher) return 0;
    const minOrder = Number(voucher.min || 0);
    if(minOrder > 0 && subtotal < minOrder) return 0;
    const type = voucher.type || 'fixed';
    const value = Math.max(0, Number(voucher.value || 0));
    const max = Math.max(0, Number(voucher.max || 0));
    let discount = 0;
    if(type === 'percent') discount = Math.floor(subtotal * Math.min(100, value) / 100);
    else if(type === 'free_shipping') discount = Number(shippingCost || 0);
    else discount = value;
    if(max > 0) discount = Math.min(discount, max);
    return Math.max(0, Math.min(discount, subtotal + Number(shippingCost || 0)));
  }

  function refreshVoucherSummary(shippingCost){
    const voucher = getSelectedVoucher();
    const discount = calculateVoucherDiscount(voucher, shippingCost);
    if(voucher && discount > 0){
      voucherSummaryLine.hidden = false;
      summaryVoucherDiscount.textContent = '- ' + rupiah(discount);
      voucherHelp.textContent = voucher.min_label + ' • voucher siap dipakai.';
    }else{
      if(voucherSummaryLine) voucherSummaryLine.hidden = true;
      if(summaryVoucherDiscount) summaryVoucherDiscount.textContent = '- Rp 0';
      if(voucherHelp){
        voucherHelp.textContent = voucher ? (voucher.min_label + ' • voucher belum memenuhi syarat checkout ini.') : (vouchers.length ? 'Pilih voucher yang sudah kamu klaim dari menu Reward.' : 'Belum ada voucher aktif. Tukar poin di menu Reward dulu.');
      }
    }
    return discount;
  }
  ...
  function selectShipping(option){
    ...
    const voucherDiscount = refreshVoucherSummary(cost);
    summaryTotal.textContent = rupiah(subtotal + cost - voucherDiscount);
    ...
    if(confirmTotal) confirmTotal.textContent = rupiah(subtotal + cost - voucherDiscount);
  ...
  if(voucherCodeSelect){
    if(selectedVoucherOld) voucherCodeSelect.value = selectedVoucherOld;
    voucherCodeSelect.addEventListener('change', function(){
      const cost = Number(shippingCostInput.value || 0);
      const voucherDiscount = refreshVoucherSummary(cost);
      summaryTotal.textContent = rupiah(subtotal + cost - voucherDiscount);
      if(confirmTotal) confirmTotal.textContent = rupiah(subtotal + cost - voucherDiscount);
    });
    refreshVoucherSummary(Number(shippingCostInput.value || 0));
  }
  if(!selectedOld){
    const payBtn = document.getElementById('openConfirmBtn');
